...
def APF_based_select(samples, x_goal):
    # 几个用于计算的相关参数
    k_att = 0.001
    k_rep = 280
    Q_star = 1000000
    ...
    repus = []
    for x in samples:
        recip_d_x_x_obs = 1 / dist(x, x_obs)
        d_x_x_goal_2 = dist(x, x_goal) ** 2
    ...
    # print("(attr,repu)=({},{})".format(attrs[min_index], repus[min_index]))
...
def generate_path_with_constraint(path, x_start, x_goal, rrt):
    ...
    for i in range(1, len(path)):
        ...
        selected_sample = APF_based_select(samples2, x_goal)
...
def main():
    ...
    rrt = Rrt(x_start, x_goal, STEP_LEN, 0.05, 10000)
    path = rrt.planning()
    if path:
