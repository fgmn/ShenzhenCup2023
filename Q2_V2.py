...
STEP_LEN = 50
TAKE_DRONE_AS_OBS = False
TURNING_CONSTRAINT = False
...
def APF_based_select(samples, x_goal, cur_pos, k_att = 10, k_rep = 1e6):
    ...
    # 仍然会走到“死胡同”!
    min_index = 0
    min_value = max(output)
    for i in range(len(output)):
        if output[i] < min_value:
            min_value, min_index = output[i], i

    # 存储后面绘制引力-斥力势能变化图
    attr_repu.append([int(attrs[min_index]), int(repus[min_index])])
    # print("(attr,repu)=({},{})".format(int(attrs[min_index]), int(repus[min_index])))
    # print("(dir,v)=({},{})".format(min_index, min_value))
...
problem list:
①APF+两个约束很难找到解 → 先忽视转弯半径约束，最后平滑处理
